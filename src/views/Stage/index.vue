            ...
            >
              <template #body="slotProps">
                <template v-if="col.data == 'id'">
                  <RouterLink :to="'/Stage/edit/' + slotProps.data[col.data]">
...
const { model, headerForm, visibleDialog } = storeToRefs(store_Stage);
...
const showing = ref([]);
const column_cache = "columns_Stage"; ////
const first = ref(0);
const rows = ref(10);
const draw = ref(0);
...
const loadLazyData = () => {
  loading.value = true;
  StageApi.table(lazyParams.value).then((res) => {
    // console.log(res);
